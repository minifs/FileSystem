				...
				for(int i=1;i<=argc-1;i++){
					if(strcmp(argv[i],">")==0&&(i+1)<=argc-1){
						...
						if(0){
							...
							strcpy(tempReturn,argv[i]);
        	                strcat(tempReturn,":\n");
        	                //strcat(tempReturn,dir_ls(absolutePath));
        	                strcat(tempReturn,"dir1\ndate.txt\ncat.mp3\n");
						}
					}
					else if(strcmp(argv[i],">")!=0){
				...
				for(int i=1;i<=argc-1;i++){
					if(strcmp(argv[i],">")==0&&i==1&&(i+1)<=argc-1){//ex:cat > test.txt
						...
						if(0){
							...
							void *buf=NULL;
							//read_file_by_path(absolutePath,&buf);
				...
				for(int i=1;i<=argc-1;i++){
					if(strcmp(argv[i],">")==0&&i==1&&(i+1)<=argc-1){//ex:rm > test.txt
						...
						if(0){
							...
							//delete_file_by_path(absolutePath);
						}
					}
						...
						if(0){
							...
							for(int k=strlen(absolutePath)-1;k>=0;k--){
				                if(absolutePath[k]=='/'&&k!=0){
					                absolutePath[k]='\0';
					                //dir_delete(absolutePath,&absolutePath[k+1]);
					                break;
				                }
				                else if(absolutePath[k]=='/'&&k==0){//已經切到root目錄了
				    	            absolutePath[k]='\0';
				    	            //dir_delete("/",&absolutePath[k+1]);
					                break;
					            }
			...
			if(argc<=2)
				...
				for(int i=1;i<=argc-1;i++){
					if(strcmp(argv[i],">")==0&&i==1&&(i+1)<=argc-1){//ex:cp > test.txt
				        ...
				        //else if(search_file(absolutePath)==0){//找到
							...
							if(cpTarget>=3){//多檔案複製
								if(strcmp(target,"")!=0){
									...
									//write_file_by_path(target,(char*)buf,len);
								}
						...
						//else if(dir_change(absolutePath)==1){//找到
						else if(1){
					...
					if(strcmp(argv[i],">")==0&&i==1&&(i+1)<=argc-1){//ex:mv > test.txt
				        ...
				        //else if(search_file(absolutePath)==0){//找到
			                ...
			                for(int k=strlen(absolutePath)-1;k>=0;k--){
				                if(absolutePath[k]=='/'&&k!=0){
					                absolutePath[k]='\0';
					                //rename_file(absolutePath,&absolutePath[k+1],argv[i+1]);
					                break;
				                }
				                else if(absolutePath[k]=='/'&&k==0){//已經切到root目錄了
				    	            absolutePath[k]='\0';
				    	            //rename_file("/",&absolutePath[k+1],argv[i+1]);
					                break;
					            }
					...
					if(strcmp(argv[i],">")==0&&i==1&&(i+1)<=argc-1){//ex:mv > test.txt
				        ...
				        //else if(search_file(absolutePath)==0){//找到
			                ...
			                for(int k=strlen(absolutePath)-1;k>=0;k--){
				                if(absolutePath[k]=='/'&&k!=0){
					                absolutePath[k]='\0';
					                //dir_rename(absolutePath,&absolutePath[k+1],argv[i+1]);
					                break;
				                }
				                else if(absolutePath[k]=='/'&&k==0){//已經切到root目錄了
				    	            absolutePath[k]='\0';
				    	            //dir_rename("/",&absolutePath[k+1],argv[i+1]);
					                break;
					            }
					...
					if(strcmp(argv[i],">")==0&&i==1&&(i+1)<=argc-1){//ex:touch > test.txt
						...
						if(0){
							for(int k=strlen(absolutePath)-1;k>=0;k--){
				                if(absolutePath[k]=='/'&&k!=0){
					                absolutePath[k]='\0';
					                //create_file(absolutePath,&absolutePath[k+1]);
					                break;
				                }
				                else if(absolutePath[k]=='/'&&k==0){//已經切到root目錄了
				    	            absolutePath[k]='\0';
				    	            //create_file("/",&absolutePath[k+1]);
					                break;
					            }
					...
					if(strcmp(argv[i],">")==0&&i==1&&(i+1)<=argc-1){//ex:mkdir > test.txt
						...
						if(0){
							for(int k=strlen(absolutePath)-1;k>=0;k--){
				                if(absolutePath[k]=='/'&&k!=0){
					                absolutePath[k]='\0';
					                //dir_create(absolutePath,&absolutePath[k+1]);
					                break;
				                }
				                else if(absolutePath[k]=='/'&&k==0){//已經切到root目錄了
				    	            absolutePath[k]='\0';
				    	            //dir_create("/",&absolutePath[k+1]);
					                break;
					            }
